    ...
    static class Program
    {
        ...
        static void Main(params string[] parameters)
        {
            if(parameters.Length > 0)
            {
                ...
                {
                    Console.WriteLine("security_authorization_filter -console <sql server> <account> <password>");
                }
                // expected parameters: -console <sql server> <account> <password>
                else if(parameters.Length == 4 && parameters[0].ToLower() == "-console")
                {
                    Console.WriteLine("\n\nSecurity Authorization Filter (CONSOLE MODE)");
                    ...
                    TrustedApplicationServer _authFilter = new TrustedApplicationServer(_entity, null, _level);

                    _authFilter.Start();

                    Console.WriteLine("\nPress escape to stop Security Authorization Server");
