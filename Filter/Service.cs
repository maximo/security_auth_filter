...
namespace security_authorization_filter
{
    ...
    {
        TrustedApplicationServer authFilter;
        ...
        protected override void OnStart(string[] args)
        {
            ...
            string customer = "";
            string licenses = "";
            string version = "version 2.7";

            if(customer == null)
            {
                // license verification.
                DateTime _expiration = new DateTime( 2018, 8 /* month */, 1 /* day */, 12, 0, 0 );
                if (DateTime.Compare( DateTime.Now, _expiration ) >= 0)
                {
                    logger.LogError( "This trial version of Security Authorization Filter has expired. Please contact www.security-filters.com to purchase a license." );
                    this.Stop();
                    return;
                }
            }
            ...
            {
                copyright += "\n\n" + version + "\n\nThe Security Authorization Filter is expressly licensed to " +
            ...
            authFilter = new TrustedApplicationServer(cEntity, logger, _level);

            Thread _thread = new Thread(new ThreadStart(authFilter.Start));
